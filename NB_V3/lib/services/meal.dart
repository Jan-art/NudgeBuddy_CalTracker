...
// Meal Service class to store the data for the meals in the meal screen. Used in the MealScreen class. It is used to add, delete, update and get the meals from the database through the use of queries.
//==================================================================================================================================================================
class MealService {
  //ADD MEAL
  ...
  Future<void> getTakenNutrients() async {
    ...
    for (int i = 0; i < meals!.docs.length; i++) {
      cont.takenProtein.value +=
          (num.parse(meals.docs[i].get('Protein').toString())*num.parse(meals.docs[i].get('Quantity'))).toInt();
